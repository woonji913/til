...
/*
    ...
    - 두번쨰 매개변수 현재 배열요소, 현재 인덱스, 배열 자체 순으로 들어간다.
*/
...
/*
    ...
    - 배열 검색 헬퍼들 : find, indexOf, lastIndecOf, findIndex, some, every
*/
...
const account = ACCOUNTS.find(function(money) {
    return money.balance === 12
})
console.log(account)
...
/*
    - 기존처럼 대상 배열에서 특정 요소를 뽑거나 배열을 return하지 않고, 
      조건에 대해 boolean 값을 return
    - some : 조건에 맞는 요소를 찾으면 즉시 검색을 멈추고 true / 찾지 못하면 flase
...
// 6-3 REQUESTS 배열에서 각 요청들 중 status 가 pending 인 요청이 있으면, 
//     inProgress 변수에 true 를 저장하라.
const REQUESTS = [
    { url: '/photos', status: 'complete' },
    { url: '/albums', status: 'pending' },
    { url: '/users', status: 'failed' },
]
const inProgress = REQUESTS.some(function (req) {
